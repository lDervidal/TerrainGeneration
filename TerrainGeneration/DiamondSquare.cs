...
namespace TerrainGeneration
{
    public struct HeightMap
    {
        ...
        public HeightMap(int width, int height) : this()
        {
    ...
    public static class DiamondSquare
    {
        public static HeightMap Generate(HeightMap baseHeights, float errorConstant, float rectangleSize)
        {
            ...
            // Создание новой карты высот с соответствующими размерами
            ...
            // Square step
            // Создаем данные средней точки (по алгоритму)
            ...
            for (int y = 0; y < newMap.Height; ++y)
            {
                ...
                for (int x = 0; x < baseHeights.Width; ++x)
                {
                    ...
                    {
                        // По алгоритму, если дословно: Берем образцы с окружающего алмаза
                        if (xCoord > 0)
                        {
                            sampleSum += 1f;
                        ...
                        // Это дает более крутой результат и уменьшает случайный диапазон
